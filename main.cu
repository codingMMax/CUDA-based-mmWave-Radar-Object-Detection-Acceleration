...
int main(int argc, char *argv[])
{
    ...
    while ((size = (int)fread(read_data, sizeof(short), data_per_frame, fp)) > 0)
    {
        ...
        cudaDist[frameCnt] = cudaDistanceDetection(fftTime, preProcessingTime, findMaxTime, totalTime, read_data, base_frame_rx0_device, frame_buffer_device, frame_reshaped_device, frame_reshaped_rx0_device, size, rx0_extended_size);
