...
int Kernel_radarDectection(double &speedTime, double &angleTime, double &distTime, double &fftTime, double &findMaxTime, double &preProcTime, char *filepath, double *Dis, double *Agl, double *Spd)
{
    // ----------------------read int16 ------------------------------
    Timer timer;
    ...
    preProcTime += (timer.elapsed() - begin);
    ...
    while (1)
    {
        ...
        // ----------------------- resahpe data ------------------------
        preProcTime += (timer.elapsed() - preProc);
        // ******************************************************** distance *****************************************************************//
        double distBegin = timer.elapsed();
        // ----------------------- FFT data ------------------------
        ...
        *(Dis + frm) = maxDis;
        distTime += (timer.elapsed() - distBegin);
        // ******************************************************** distance *****************************************************************//
...
int main()
{
    ...
    // testOMP();
    int frm;
    double angleTime = 0, speedTime = 0, distTime = 0, preProcessingTime = 0, fftTime = 0, findMaxTime = 0;
    Timer timer;
    double totalTime = timer.elapsed();
    frm = Kernel_radarDectection(speedTime, angleTime, distTime, fftTime, findMaxTime, preProcessingTime, filepath, Dis, Agl, Spd);
    totalTime = timer.elapsed() - totalTime;
    int frameCnt = Framenum - 1;
    printf("CPU Benchmark totalTime %.3f ms %.3f ms per frame FPS %.3f\n", 1000.0 * totalTime, 1000.0 * totalTime / frameCnt, frameCnt / totalTime);
    printf("CPU Benchmark total fft time %.3f ms, %.3f ms per frame FPS %.3f \n", 1000.0 * fftTime, 1000.0 * fftTime / frameCnt, frameCnt / fftTime);
    printf("CPU Benchmark total preProcessing time %.3f ms, %.3f ms per frame FPS %.3f \n", 1000.0 * preProcessingTime, 1000.0 * preProcessingTime / frameCnt, frameCnt / preProcessingTime);
    printf("CPU Benchmark total findMaxTime time %.3f ms, %.3f ms per frame FPS %.3f \n", 1000.0 * findMaxTime, 1000.0 * findMaxTime / frameCnt, frameCnt / findMaxTime);
    printf("CPU Benchmark total distance calculation time %.3f ms, %.3f ms per frame FPS %.3f \n", 1000.0 * distTime, 1000.0 * distTime / frameCnt, frameCnt / distTime);
    printf("CPU Benchmark total angle calculation time time %.3f ms, %.3f ms per frame FPS %.3f \n", 1000.0 * angleTime, 1000.0 * angleTime / frameCnt, frameCnt / angleTime);
    printf("CPU Benchmark total speed calculation time time %.3f ms, %.3f ms per frame FPS %.3f \n", 1000.0 * speedTime, 1000.0 * speedTime / frameCnt, frameCnt / speedTime);
    // testMatrixTranspose();
    // testFFTshift();
    free(Dis);
    free(Agl);
