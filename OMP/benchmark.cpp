...
int DectectDis(char *filepath, double *Dis){
    ...
    // printf("\ncnt=%d, totaltime=%fms, read=%fms,  reshape=%fms,  fft=%fms,  findmax=%fms\n",
    // cnt ,  totaltime/cnt,
    // readtime/cnt,  rptime/cnt,  
    // ffttime/cnt , maxtime/cnt); 
    printf("In kernel function: cnt=%d, totaltime=%fms, thp=%ffrm/s read=%fms,  reshape=%fms,  fft=%fms,  findmax=%fms\n",
...
int main(){
    ...
    printf("Out of kernel function: total latency= %lfms, total throughput=%lffrm/s \n",
    (cost/CLOCKS_PER_SEC*1000), 1/(cost/(frm)/CLOCKS_PER_SEC));
    free(Dis);
}
